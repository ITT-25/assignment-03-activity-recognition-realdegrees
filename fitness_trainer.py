...
class FitnessTrainer(Window):
    def __init__(
        ...
        self.stage_display.set_data(
            session.stages[self.current_stage])  # Set the first stage
    ...
    def update(self, dt: float):
        ...
        if self.stage_display.is_complete():
            ...
            self.stage_display.set_data(
                self.session.stages[self.current_stage])
...
def main(data_dir: str, model_output: str, port: int, session_file: str):
    ...
    try:
        ...
        print(
            f"Unable to load model from {model_output}. Training a new model.")
